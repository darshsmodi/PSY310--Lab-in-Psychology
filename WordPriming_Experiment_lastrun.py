...
expName = 'WordPriming_Experiment'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Darsh Modi\\OneDrive\\Documents\\AU\\PSY310\\WordPriming\\WordPriming_Experiment_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Routine1" ---
    ...
    # --- Initialize components for Routine "Routine2" ---
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('C:/Users/Darsh Modi/OneDrive/Documents/AU/PSY310/WordPriming/ConditionsFile_R1.csv'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "Routine1" ---
        # create an object to store info about Routine Routine1
        Routine1 = data.Routine(
            name='Routine1',
            components=[StudyText, Studyslider],
        )
        Routine1.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        StudyText.setText(Study_word)
        Studyslider.reset()
        # store start times for Routine1
        Routine1.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Routine1.tStart = globalClock.getTime(format='float')
        Routine1.status = STARTED
        thisExp.addData('Routine1.started', Routine1.tStart)
        Routine1.maxDuration = None
        # keep track of which components have finished
        Routine1Components = Routine1.components
        for thisComponent in Routine1.components:
        ...
        # --- Run Routine "Routine1" ---
        Routine1.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            ...
            if not continueRoutine:  # a component has requested a forced-end of Routine
                Routine1.forceEnded = routineForceEnded = True
                break
        ...
        # --- Ending Routine "Routine1" ---
        for thisComponent in Routine1.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Routine1
        Routine1.tStop = globalClock.getTime(format='float')
        Routine1.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Routine1.stopped', Routine1.tStop)
        trials.addData('Studyslider.response', Studyslider.getRating())
        trials.addData('Studyslider.rt', Studyslider.getRT())
        # the Routine "Routine1" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
    ...
    trials_2 = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('ConditionsFile_R2.csv'), 
        seed=None, 
    )
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "Routine2" ---
        # create an object to store info about Routine Routine2
        Routine2 = data.Routine(
            name='Routine2',
            components=[TestText, textbox, submit_text, testmouse],
        )
        Routine2.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        # store start times for Routine2
        Routine2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Routine2.tStart = globalClock.getTime(format='float')
        Routine2.status = STARTED
        thisExp.addData('Routine2.started', Routine2.tStart)
        Routine2.maxDuration = None
        # keep track of which components have finished
        Routine2Components = Routine2.components
        for thisComponent in Routine2.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Routine2" ---
        Routine2.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                Routine2.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in Routine2.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "Routine2" ---
        for thisComponent in Routine2.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Routine2
        Routine2.tStop = globalClock.getTime(format='float')
        Routine2.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Routine2.stopped', Routine2.tStop)
        ...
        # the Routine "Routine2" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
